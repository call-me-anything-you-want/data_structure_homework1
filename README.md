...
4. command模式
	...
	* :su/[s1]/[s2]/[flag]，同:s/[s1]/[s2]/[flag]
	* :subsitiute/[s1]/[s2]/[flag]，同:s/[s1]/[s2]/[flag]
...
值得一提的是，我们还进行了大文件测试，即使用SimpleWord尝试打开一个1.32GB，9397022行的大文件（记事本无法打开），发现可以正常打开，且在查看过程中无明显卡顿现象。

但有以下问题：
1. 无法打开超大的文件
	* 原因：每次打开文件时，该程序都需要将文件完整地读入文件缓冲区，而缓冲区的大小受限于电脑内存大小。
	* 可能的解决方案：内存映射
2. 无法进行过于复杂的编辑
	* 原因：撤销操作需要保存整个文件缓冲区，带来比较大的内存开销
	* 可能的解决方案：限制撤销最大步数，或改变文件缓冲区的数据结构，如使用Piece Table
3. 不支持英文以外的文件内容
	* 原因：std::string对其它文字的不完善支持
	* 可能的解决方案：实现自己的string，使之支持宽字符
4. 块选择模式下无法高亮选择内容
	* 原因：不会
	* 可能的解决方案：暂无

### 结论
虽然仍有待解决的问题，此程序总体来说较完善地满足了课程设计要求
